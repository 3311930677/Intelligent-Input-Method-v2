...
int main() {
    ...
    const auto path = std::filesystem::temp_directory_path() / "owo-engine-benchmark.owolx";
    const auto written = owo::engine::write_binary_lexicon(path, std::move(entries));
    ...
    constexpr std::string_view queries[]{"nihao", "xian", "ceshi", "zhongguo"};
    ...
    std::cout << "{\"benchmark\":\"owo.engine.candidate_generation\""
              ...
              << "Debug"
#endif
              << "\",\"samples\":1000,\"warmup\":100,\"noise_entries\":" << noise_entries
              ...
              << ",\"max\":" << samples.back() << "}}\n";
    return total_candidates == 1500 ? 0 : 3;
}
