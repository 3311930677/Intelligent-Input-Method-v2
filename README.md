...
TSF 捕获英文字母形成临时预编辑缓冲，并在后台请求基础候选与可选智能增量；`1` 或空格提交，Backspace 删除，Escape 取消。候选窗保留为 TSF 进程内的轻量 Win32 非激活窗口，使用 Direct2D/DirectWrite 绘制两行布局：首行忠实显示用户输入并以 `'` 标出 Core 推断的分段（不会显示词典补全后的拼音），次行显示可点击候选、翻页和展开表格按钮。窗口优先定位到文本光标，无法取得范围时回退到鼠标位置。
