...
当本机 Visual Studio 版本与 `v143` 预设不匹配时，可使用不绑定 VS 版本的 Ninja Debug 预设：
...
- `owo_emoji_plugin` 是零权限 AppContainer 插件。输入 `/smile`、`/happy`、`/sad`、`/love`、`/cry` 等显式命令时，Core 经 `PluginExecutor` 调用 `emoji.query.v1`，再把返回值注入候选窗。开发包可用 `scripts/build-emoji-plugin.ps1 -Install` 签名安装；测试结束执行 `scripts/remove-dev-plugin-cert.ps1` 撤销临时开发信任。
- `owo_voice_input_plugin` 是独立语音模块，使用 OWVH v1 协议和 Windows SAPI；`--once` 可单独验证麦克风识别。其 `microphone.capture` 与 `text.commit.v1` 能力接入仍必须经过后续 L3 broker，不会绕过 TSF/Core 安全边界。
- `modules/ai-text` 保留独立 AI 模块的下载与授权边界。真实模型或云端 API 后端尚未随仓库分发，不把 mock 推理描述为已完成 AI 插件。

插件平台分级和 L3 协议草案见 `docs/architecture/plugin-platform-design.md` 与 `docs/architecture/l3-capability-protocol.md`。
