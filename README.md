...
TSF 捕获英文字母形成临时预编辑缓冲，并在后台请求基础候选与可选智能增量；`1` 或空格提交，Backspace 删除，Escape 取消。候选窗保留为 TSF 进程内的轻量 Win32 非激活窗口，使用 Direct2D/DirectWrite 绘制，并优先定位到文本光标；无法取得范围时回退到鼠标位置。
